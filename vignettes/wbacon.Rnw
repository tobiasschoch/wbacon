...
to allow for weighting is due to \citet{beguin_hulliger_2008}.
...
Users of Microsoft Windows need an installation of the the R tool chain bundle
...
\noindent The instance \code{reg} is an object of the class \code{roblm}.
...
for objects of the class \code{roblm} because it is not sensible to study
