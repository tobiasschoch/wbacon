...
Table 1. *Benchmarks: Robust linear regression (single thread): Ratio of average compute time. A ratio $> 1.0$ ($< 1.0$) implies that `wBACON_reg()` is faster (slower) than `robustX:BACON()`*


In the second benchmark, we study the ratio of average compute time of `wBACON()` vs. `robustX::BACON()` for multivariate outlier nomination; see Table 2. A ratio $> 1.0$ ($< 1.0$) implies that `wBACON()` is faster (slower) than `robustX:BACON()`. The number of variables ($p$) and the number of observations ($n$) are varied. More importantly, `wBACON()` is set up with thread-level and instruction-level parallelization: [OpenBLAS](http://www.openblas.net) in place of the standard `BLAS` library and full OpenMP [@openmp2018] support. For ease of simplicity, we use a "plain vanilla" parallelization mode which spawns all available cores/ threads.

For very small data sets (e.g., $n=10^3$ and $p=5$), `wBACON()` is slower because parallelization leads to computation overhead that dominates computation time. Clearly, it would be more efficient to specify only 1 or 2 threads for such small data. However, the differences in computing time are hardly noticeable to the user (0.08 vs. 0.14 seconds). For larger data sets (in terms of number of variables and observations), `wBACON()` easily outperforms `robustX:BACON()`; see Table 2. The differences in compute time between the two implementations become larger as we increase $n$ or $p$.
...
Table 2. *Benchmarks: Multivariate outlier nomination/ detection (multithreading): Ratio of average compute time. A ratio $> 1.0$ ($< 1.0$) implies that `wBACON()` is faster (slower) than `robustX:BACON()`*
