...
\end{verbatim}
\noindent where \REGOMPMINSIZE, and \code{n} and \code{p} are, respectively,
...
using the OpenMP preprocessor directive
\begin{verbatim}
#pragma omp parallel for if(p * n > FITLS_OMP_MIN_SIZE)
\end{verbatim}
\noindent where \FITWLSOMPMINSIZE, and \code{n} and \code{p} are, respectively,
the number of rows and columns.
