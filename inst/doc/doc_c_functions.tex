...
%
% fitwls -> arguments
% add more details to the 'internals' of the functions
%
...
\begin{Arguments}
	\begin{ldescription}
		...
		\MAXITER{, default \code{maxiter = 50}.}
		\VERBOSE
...
\begin{Arguments}
	\begin{ldescription}
		...
		\VERBOSE
		\item[\code{success}] \code{0}: successful termination; \code{1}: error,
...
\end{description}
The function returns an \code{\LinkA{wbacon\_error\_type}{wbaconerrortype}}.
\end{Value}
...
\end{Dependencies}
\begin{Value}
On return, \code{dat->dist} is overwritten with the Mahalanobis distance. The
function returns an \code{\LinkA{wbacon\_error\_type}{wbaconerrortype}}.
\end{Value}
...
\end{Arguments}
\begin{Value}
On return, \code{center} is overwritten with the computed center.
\end{Value}
...
\end{Dependency}
\begin{Value}
On return, \code{scatter} is overwritten with the computed scatter matrix.
\end{Value}
...
To offer functions with a clean interface, most of the functions use the typedef
structs \code{\LinkA{regdata}{regdata}} (see \code{regdata.h}),
...
struct \code{regdata} are considered immutable, with one expection: \code{wx}
and \code{wy} will be modified.

%---------------------------------------
\HeaderA{estimate}{Estimates \code{[typedef struct]}}{estimate}
\begin{ldescription}
	\item[\code{sigma}]	pointer to the regression scale, \code{double}.
	\item[\code{weight}] pointer to the weights, \code{double array[n]}. 
	\item[\code{resid}]	pointer to the residuals, \code{double array[n]}.
	\item[\code{beta}] pointer to the regression coefficient, \code{double array[p]}.
...
\begin{Description}
Selects the smallest \code{1..m} observations (w.r.t. \code{a}) into the subset.
\end{Description}
\begin{Usage}
\begin{verbatim}
...
the elements of \code{a} in ascending order. Then, the smallest $m$ observations
are selected into \code{subset}.
\end{Details}
\begin{Value}
...
\end{Arguments}
\begin{Details}
The function calls \code{\LinkA{hat\_matrix}{hatmatrix}} to compute the diagonal
elements of the ``hat'' matrix and computes the regression scale. Then,
it computes the $t_i$'s.
\end{Details}
\begin{Value}
On return, \code{tis} is overwritten with the computed $t_i$'s. The function
returns an \code{\LinkA{wbacon\_error\_type}{wbaconerrortype}}.
\end{Value}

%---------------------------------------
\HeaderB{cholesky\_reg}{Internal function}{choleskyreg}
\begin{Description}
Compute the regression coefficient using the Cholesky factor $L$ and the matrix
$X^Ty.$
\end{Description}
\begin{Usage}
\begin{verbatim}
...
\end{Arguments}
\begin{Value}
On return, \code{beta} is overwritten with the updated coefficients.
\end{Value}
...
\end{Arguments}
\begin{Value}
On return, \code{hat} is overwritten with the diagonal elements of the ``hat''
matrix. The function returns an 
\code{\LinkA{wbacon\_error\_type}{wbaconerrortype}}.
\end{Value}
