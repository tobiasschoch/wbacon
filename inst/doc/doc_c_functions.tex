...
\end{Arguments}
\begin{Value}
On return, \code{center} is overwritten with the weighted mean.
\end{Value}
...
\begin{Usage}
\begin{verbatim}
void weightedscatter(wbdata *dat, double *work, double *center,
    double *scatter)
\end{verbatim}
\end{Usage}
\begin{Arguments}
...
\end{Dependency}
\begin{Value}
On return, \code{scatter} is overwritten with the weighted scatter matrix.
\end{Value}

%---------------------------------------
\HeaderB{euclidean\_norm2}{Internal function}{euclideannorm2}
\begin{Description}
Computes the squared Euclidean norm $\Vert x_i - c \Vert_2^2$, where $c$
denotes the center.
\end{Description}
\begin{Usage}
\begin{verbatim}
