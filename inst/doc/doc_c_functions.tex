...
\def\REGOMPMINSIZE{
    \code{REG\_OMP\_MIN\_SIZE} is of size 1\,000\,000
}
\def\FITWLSOMPMINSIZE{
    \code{FITWLS\_OMP\_MIN\_SIZE} is of size 1\,000\,000
}
\def\OMPTHREADS{
    \item[\code{threads}] requested number of threads (OpenMP), \code{[int]}.
}
...
document ``methods.pdf'' (see package folder \code{doc}).
...
initialize the subset (see \code{\LinkA{initial\_subset}{initialsubset}}).
\end{Description}
\begin{Usage}
\begin{verbatim}
...
\begin{Dependencies}
	\begin{description}
		\item[internal:]
        \code{\LinkA{initial\_location}{initiallocation}},
		\code{\LinkA{initial\_subset}{initialsubset}},
...
\begin{Arguments}
	\begin{ldescription}
		...
		\MAXITER{.}
        \item[\code{original}] \code{1}: the subset of the
        \code{m = collect * p} smallest observations (small w.r.t. to the
        Mahalanobis distances) is taken from the subset generated by
        Algorithm 3 as the basic subset for regression
        [this is the original method of Billor et al. (2000)]; otherwise
        (i.e., when \code{0}) the subset that results from
        Algorithm 3 of Billor et al. (2000) is taken to be the basic subset
        for regression, \code{[int]}.
        \OMPTHREADS
	\end{ldescription}
\end{Arguments}
\begin{Details}
...
%---------------------------------------
\HeaderB{initial\_location}{Internal function}{initiallocation}
\begin{Description}
Computes the initial location: either version ``v1'' or ``v2'' or Billor
et al. (2000).
\end{Description}
\begin{Usage}
\begin{verbatim}
...
\end{Arguments}
\begin{Dependency}
    \code{\LinkA{wquantile\_noalloc}{wquantile}},
    \code{\LinkA{euclidean\_norm2}{euclideannorm2}}, and
    \code{\LinkA{mahalanobis}{mahalanobis}}
\end{Dependency}
\begin{Value}
...
error handed over by \code{\LinkA{mahalanobis}{mahalanobis}}.
...
%---------------------------------------
\HeaderB{initial\_subset}{Internal function}{initialsubset}
\begin{Description}
Computes the initial subset. This is a weighted variant of Algorithm 2 of
Billor et al. (2000).
\end{Description}
\begin{Usage}
\begin{verbatim}
...
\end{verbatim}
\noindent where \OMPMINSIZE, and \code{n} and is
the number of rows. The inner loop over the \code{n}
...
#pragma omp parallel for if(n > OMP_MIN_SIZE)
\end{verbatim}
\noindent where \OMPMINSIZE and \code{n} is
the number of rows. The inner loop over the \code{n}
rows is equipped with the directive \code{\#pragma omp simd} to tell the
compiler that we demand SIMD vectorization.
\end{Details}
...
\end{verbatim}
\noindent where \REGOMPMINSIZE and \code{n} is the number of rows and columns.
...
\end{Dependency}
\begin{Value}
The function's loop over the columns of the hat matrix is parallelized
using the OpenMP preprocessor directive
\begin{verbatim}
#pragma omp parallel for if(n > REG_OMP_MIN_SIZE)
\end{verbatim}
...
\end{verbatim}
\noindent where \FITWLSOMPMINSIZE and \code{n} is the number of rows.
...
\end{Dependencies}
\begin{Value}
The function \code{fitwls} returns its status \code{info}; if successful,
\code{info=0}; otherwise the computation failed. On return, the
following slots of \code{struct estimate est} are overwritten:
	\begin{ldescription}
		\item[\code{beta}] regression coefficients
        \item[\code{sigma}] regression scale
		\item[\code{resid}] residuals
	\end{ldescription}
and, the following slots of \code{struct regdata dat} are overwritten:
	\begin{ldescription}
		\item[\code{wx}] the QR factorization as returned by the
